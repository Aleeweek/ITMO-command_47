...
```
CREATE OR REPLACE FUNCTION CHECK_ORDER_DATE_NOT_IN_PAST()
RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION CHECK_ORDER_DATE_NOT_IN_PAST();
```
...
BEGIN
    ...
    ELSIF NEW.TYPE = 'списание' THEN
        ...
        IF (SELECT QUANTITY FROM CAR_PARTS WHERE ID = NEW.CAR_PART_ID) < 0 THEN
...
AFTER INSERT ON STORAGE_TRANSACTIONS
FOR EACH ROW
...
BEGIN
  SELECT SUM(S.PRICE)
  INTO RESULT
  FROM ORDERS_TO_SERVICES OTS
  JOIN SERVICES S ON OTS.SERVICE_ID = S.ID
...
$$ LANGUAGE PLPGSQL;
```
### Скалярная функция 3. Вычисляет среднее арифметическое всех оценок из отзывов.
```
...
$$ LANGUAGE PLPGSQL;
```
### Представление 1. Детальная информация о заказах с итоговыми суммами.
```
...
### Представление 2. Статистика по отзывам и рейтингам клиентов
```
CREATE OR REPLACE VIEW CLIENTS_RATINGS AS
SELECT
    C.ID AS CLIENT_ID,
    C.FIRST_NAME || ' ' || C.LAST_NAME AS CLIENT_NAME,
...
### Представление 3. Отслеживание занятости сотрудников.
```
CREATE VIEW EMPLOYEES_AVAILABILITY AS
SELECT
    e.id AS employee_id,
    e.first_name,
    e.last_name,
...
FROM employees e
LEFT JOIN orders o ON e.id = o.employee_id AND o.status IN ('в работе', 'ожидание');
```
 
Часть 5. Индексы и ограничения

### Индекс 1. Гарантирует уникальность комбинации имени и фамилии сотрудника.
```
CREATE UNIQUE INDEX ON EMPLOYEES (FIRST_NAME, LAST_NAME);
```

### Индекс 2. Обеспечивает уникальность связи заказ-услуга.
```
CREATE UNIQUE INDEX IDX_ORDERID_TO_SERVICEID ON ORDERS_TO_SERVICES (ORDER_ID, SERVICE_ID);
```

### Индекс 3. Гарантирует уникальность номеров автомобилей
```
CREATE UNIQUE INDEX ON CARS (NUMBER);
```
